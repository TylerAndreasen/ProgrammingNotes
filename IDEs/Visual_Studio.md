...
I have, of course, run into issues with software when I am first starting out with it and intend to write down the things I learn so that I don't forget them.
...
I am finding it surprisingly annoying to not have line numbers in Visual Studio. I would not think this little feature would be so impactful, nor would I have expected line numbers being disabled by default. This can be changed via Tools > Options > Text Editor > General, and checking the Line Numbers box. (On my desktop installation, and possibly laptop installation, this setting showed as on by default but was not reflected in the editor.)

### **4. Uesful Hotkeys**

This section will likely be expanded overtime as I learn new hotkeys.

`F5` - Run and Debug Current file.
`Ctrl+F5` - Run Current File.
`F10` - Step Over line if tracing has been started. Start Tracing if execution is not running.
`F11` - Step Into Line.
`Shfit+F11` - Step Out.
